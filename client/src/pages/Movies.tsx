...
const Movies: React.FC = () => {
  ...
  const { setMovieId } = useNetflixContext();
    ...
    const getMovies = async () => {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/movie/popular?api_key=${token.API}&language=en-EN&page=${currentPage}`
        );
      ...
      <div className="w-[90%] max-w-[1200px]">
        ...
        <div className="grid grid-cols-1 gap-8 mt-8 justify-items-center md:grid-cols-2 lg:grid-cols-4">
          {movies.map((item, index) => (
            <MoviesSingle key={index} item={item} findId={findId} />
          ))}
        </div>
